...
const Disease = () => {
  ...
  let url = "http://127.0.0.1:8000/disease-predict/" + lang;
    ...
    try {
      fetch(url, {
        ...
        .then((data) => {
          ...
          setCauses(
            main_data["info"]["causes"]["1"] +
              " " +
              main_data["info"]["causes"]["2"]
          );
          setPrevent(
            main_data["info"]["prevent"]["1"] +
              " " +
              main_data["info"]["prevent"]["2"] +
              " " +
              main_data["info"]["prevent"]["3"]
          );
      ...
      <section className="">
        <div className="grid place-items-center my-14  ">
          <div className="container bg-gray-100  p-10 grid place-items-center my-14 ">
            <p className="text-2xl font-medium text-lime-600 my-12">
            ...
            <button
              onClick={() => onClick()}
              type="button"
              className="inline-block px-6 py-2.5 bg-lime-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out"
            >
              Get Upload
            </button>
        ...
        <div>
          ...
          {prediction !== "" ? (
            <div className="grid place-items-center my-14 text-center ">
              ...
              {prediction}
              <p className="font-bold my-3">Causes: </p>
              {causes}
              <p className="font-bold my-3">Prevention: </p>
              {prevent}
            </div>
          ) : (
            <div></div>
